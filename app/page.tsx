...
const features = [
  {
    ...
    icon: (
      <svg width="22" height="22" fill="none" stroke="#d81b8a" strokeWidth="1.5" viewBox="0 0 24 24">
...
export default async function HomePage() {
  ...
  const [products, featured, { data: heroData }] = await Promise.all([
    getProducts(),
    getFeaturedProducts(),
    supabase.from('hero_images').select('url').order('position'),
  ])
  const preview = products.slice(0, 3)
  const heroImages = (heroData ?? []).map((h) => h.url)
  const hasHero = heroImages.length > 0
  const waUrl = `https://wa.me/${WA_NUMBER}?text=${encodeURIComponent('Hola PinsCrew! Quiero consultar sobre un proyecto.')}`
  ...
  return (
    <SiteShell>
      ...
      >
        ...
        >
          <span className="mb-10 block h-px w-10 mx-auto" style={{ backgroundColor: '#d81b8a' }} aria-hidden="true" />
          ...
          <Link href="/productos" className="pc-btn mt-10 px-6 py-3 text-[14px]">
            Ver productos →
          </Link>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1200px] px-6 py-24">
        <div className="flex items-baseline justify-between mb-10" style={{ borderBottom: '1px solid #e5e5e5', paddingBottom: '20px' }}>
          ...
          <Link href="/productos" className="text-[13px]" style={{ color: '#6b6b6b' }}>
            Ver todos →
          </Link>
        </div>
        {preview.length > 0 ? (
